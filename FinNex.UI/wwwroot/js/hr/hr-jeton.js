    ...
    document.getElementById('hjRedimModalBody').innerHTML = `
        <div class="hj-redim-info">
            ...
            ${(r.redimNovu === 1 && r.icazeSaati > 0) ? `
                ...
                : `${r.icazeTarixi ? `<div class="hj-redim-row"><strong>Tarix</strong><span>${hjDate(r.icazeTarixi).split(',')[0]}</span></div>` : ''}
                   <div class="hj-redim-row"><strong>Cəmi saat (jeton)</strong><span><b>${r.cemiSaat} saat</b></span></div>`}
    ...
    try {
        ...
        tbody.innerHTML = json.data.map(r => `
            <tr>
                ...
                <td><strong>${(r.redimNovu === 1 && r.icazeSaati > 0) ? r.icazeSaati : r.cemiSaat} saat</strong>${(r.redimNovu === 1 && r.icazeSaati > 0) ? `<br><small style="color:#6b7280">jeton: ${r.cemiSaat} saat</small>` : ''}</td>
                <td>${hjDate(r.telabTarixi)}</td>
