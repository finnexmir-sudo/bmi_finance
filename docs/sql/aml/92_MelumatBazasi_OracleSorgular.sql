/* ============================================================================
   ...
   ⚠️ 11 sorğudan 3-ü DÖVRSÜZDÜR (token yoxdur) — BMI-də də belədir:
...
DECLARE @DepId INT =
    (SELECT TOP 1 Id FROM Departamentler
      WHERE REPLACE(LOWER(Ad), N' ', N'') LIKE N'%risk%' AND ISNULL(Silinib,0) = 0);

DECLARE @Kataloq NVARCHAR(100) = N'AML / Məlumat Bazası';
