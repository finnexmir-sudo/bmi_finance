/* ============================================================================
   AML — «Öz xeyrinə» (AQ/AR) sütunları TƏYİNAT MƏTNİNDƏN də doldurulsun
   ----------------------------------------------------------------------------
   NƏ EDİR: `OracleSorgular` cədvəlindəki İKİ mövcud sətrin `SorguMetni`-ni
   YENİLƏYİR (`AML_HESAB_SORGU_FIZIKI`, `AML_HESAB_SORGU_HUQUQI`).
   ...
   NƏ DƏYİŞİR: AQ («Xeyrinə ödənilən şəxs/müəssisə») və AR («FİN/VÖEN»).
   Əvvəl YALNIZ `doc_vnesh_postupl` / `doc_vnesh_swift` sahələrindən dolurdu —
   yəni yalnız MƏDAXİL sətirlərində. İndi HƏR İKİSİ boş olduqda təyinat
   mətnindən oxunur:

       VÖEN-AVANS: 1604964601 - Dövlət Gömrük Komitəsi (...)
                   |__ AR __|   |________ AQ _________|

   NƏ DƏYİŞMİR: struktur mənbədən gələn dəyər HEÇ VAXT üstələnmir; şablon
   tapılmasa sütun bu günkü kimi boş qalır. Dəyişiklik yalnız ƏLAVƏ EDİR.

   ⚠️ ƏVVƏLCƏ ÖLÇ: `04_Teyinat_Voen_Diaqnostikasi.sql` (PL/SQL Developer-də) —
   şablonun real datada nə qədər tutduğunu və yalançı pozitiv olub-olmadığını
   göstərir.
...
SELECT SorguAdi,
       ...
       CASE WHEN SorguMetni LIKE N'%teyinat_voen%'
...
/* -- YOXLAMA — her ikisinde «teyinat_voen» olmalidir ---------------------- */
SELECT SorguAdi,
