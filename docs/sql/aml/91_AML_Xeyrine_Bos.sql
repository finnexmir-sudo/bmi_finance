/* ============================================================================
   AML — «Öz xeyrinə» (AQ/AR) sütunları BOŞALDILIR
   ----------------------------------------------------------------------------
   ...
   QƏRAR (istifadəçi, 20.08.2026): «hesabatda o iki sütuna biz heç nə yazmırıq —
   öz xeyrinə ad və VÖEN. Boş qoyuruq.»

   Əvvəl AQ/AR YALNIZ mədaxil sətirlərində dolurdu (`doc_vnesh_postupl` →
   kredit_name/kredit_inn, `doc_vnesh_swift` → beneficiary_bank_bic); məxaric
   və daxili sətirlərdə boş qalırdı. Yarımçıq sütun AML hesabatında yanıldıcıdır —
   oxuyan boş xananı «üçüncü tərəf yoxdur» kimi başa düşür. İndi sütun TAM boşdur.

   NƏ DƏYİŞMİR: sütun sayı (47), sıra, Excel şablonu ilə uyğunluq. Yalnız iki
   xana boş gəlir. Alt qatdakı `vd.xeyrine_ad` / `vd.xeyrine_fin` sahələri
   YERİNDƏ QALIR — qayda dəqiqləşəndə bərpa iki sətirlik dəyişiklikdir.
...
SELECT SorguAdi,
       ...
       CASE WHEN SorguMetni LIKE N'%cast(null as varchar2(300))%'
            THEN N'artiq bosaldilib' ELSE N'bosaldilacaq' END   AS Veziyyet
  FROM OracleSorgular
...
/* -- YOXLAMA — her ikisinde AQ/AR bos olmalidir --------------------------- */
SELECT SorguAdi,
