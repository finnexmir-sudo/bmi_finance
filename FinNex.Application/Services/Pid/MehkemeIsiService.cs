...
public class MehkemeIsiService : IMehkemeIsiService
{
    ...
    public async Task<IList<MehkemeIsiCourtListDto>> MehkemeSiyahisiAsync(string? axtaris = null)
    {
        var q = _uow.Repository<MehkemeIsi>().Query().Where(x => !x.Silinib);
        if (!string.IsNullOrWhiteSpace(axtaris))
        {
            var a = axtaris.Trim();
            q = q.Where(x => x.BorcluAd.Contains(a)
                          || (x.KreditHesabi != null && x.KreditHesabi.Contains(a))
                          || (x.QeydiyyatNomresi != null && x.QeydiyyatNomresi.Contains(a)));
        }
        var list = await q.Include(x => x.Merheleler)
            .OrderBy(x => x.Sira).ThenByDescending(x => x.Id).AsNoTracking().ToListAsync();

        return list.Select(x => new MehkemeIsiCourtListDto
        {
            Id = x.Id,
            Sira = x.Sira,
            Status = !string.IsNullOrWhiteSpace(x.MehkemeStatusMetn) ? x.MehkemeStatusMetn : x.Status.ToString(),
            BorcluAd = x.BorcluAd,
            KreditNovu = x.KreditNovuMetn,
            KreditHesabi = !string.IsNullOrWhiteSpace(x.KreditHesabi) ? x.KreditHesabi : x.QeydiyyatNomresi,
            Subkod = !string.IsNullOrWhiteSpace(x.Subkod) ? x.Subkod : x.KreditSubHesab,
            MehkemeyeVerilmeTarixi = x.MehkemeyeVerilmeTarixi,
            MehkemeSenedi = x.MehkemeSenedi,
            QetnameTarixi = x.QetnameTarixi,
            Iclaslar = x.Merheleler.Where(m => !m.Silinib).OrderBy(m => m.Tarix)
                .Select(m => new MehkemeCedvelIclasDto { Id = m.Id, Tarix = m.Tarix, Saat = m.Qeyd }).ToList()
        }).ToList();
    }

    public async Task<(int isSayi, int iclasSayi)> MehkemeImportAsync(List<MehkemeCedvelImportDto> isler, int isciId)
    {
        if (isler == null || isler.Count == 0) return (0, 0);
        var movcud = await _uow.Repository<MehkemeIsi>().Query().Where(x => !x.Silinib).ToListAsync();
        int isS = 0, iclasS = 0;
        foreach (var d in isler)
        {
            if (string.IsNullOrWhiteSpace(d.BorcluAd)) continue;
            var ad = d.BorcluAd.Trim();
            var e = movcud.FirstOrDefault(m => string.Equals(m.BorcluAd != null ? m.BorcluAd.Trim() : null, ad, StringComparison.OrdinalIgnoreCase));
            bool yeni = e == null;
            if (yeni)
                e = new MehkemeIsi { QeydiyyatNomresi = "", BorcluAd = ad, YaradanIcraciId = isciId, YaradilmaTarixi = DateTime.Now };

            if (d.Sira.HasValue) e!.Sira = d.Sira;
            if (!string.IsNullOrWhiteSpace(d.Status)) e!.MehkemeStatusMetn = d.Status.Trim();
            if (!string.IsNullOrWhiteSpace(d.KreditNovu)) e!.KreditNovuMetn = d.KreditNovu.Trim();
            if (d.MehkemeyeVerilmeTarixi != null) e!.MehkemeyeVerilmeTarixi = d.MehkemeyeVerilmeTarixi;
            if (!string.IsNullOrWhiteSpace(d.MehkemeSenedi)) e!.MehkemeSenedi = d.MehkemeSenedi.Trim();

            foreach (var ic in d.Iclaslar)
            {
                if (ic.Tarix == null) continue;
                e!.Merheleler.Add(new MehkemeMerhelesi { MerheleTipi = MerheleTipi.MehkemeIclasi, Tarix = ic.Tarix.Value, Qeyd = ic.Saat, YaradilmaTarixi = DateTime.Now });
                iclasS++;
            }

            if (yeni) { await _uow.Repository<MehkemeIsi>().YaratAsync(e!); movcud.Add(e!); }
            else { await _uow.Repository<MehkemeIsi>().YenileAsync(e!); }
            isS++;
        }
        await _uow.YaddaSaxlaAsync();
        return (isS, iclasS);
    ...
    public async Task<int> MehkemeYaratAsync(MehkemeCedvelCreateDto dto, int isciId)
    {
        var e = new MehkemeIsi
        {
            QeydiyyatNomresi = (dto.KreditHesabi ?? "").Trim(),
            KreditSubHesab = dto.Subkod?.Trim(),
            KreditHesabi = dto.KreditHesabi?.Trim(),
            Subkod = dto.Subkod?.Trim(),
            BorcluAd = (dto.BorcluAd ?? "").Trim(),
            Sira = dto.Sira,
            MehkemeStatusMetn = dto.Status?.Trim(),
            KreditNovuMetn = dto.KreditNovu?.Trim(),
            MehkemeyeVerilmeTarixi = ParseTarix(dto.MehkemeyeVerilmeTarixi),
            MehkemeSenedi = dto.MehkemeSenedi?.Trim(),
            QetnameTarixi = ParseTarix(dto.QetnameTarixi),
            Qeyd = dto.Qeyd?.Trim(),
            YaradanIcraciId = isciId,
            YaradilmaTarixi = DateTime.Now
        };
        if (dto.IclasTarix != null)
        {
            ...
            {
                var t = ParseTarix(dto.IclasTarix[i]);
                if (t == null) continue;
                var saat = (dto.IclasSaat != null && i < dto.IclasSaat.Count) ? dto.IclasSaat[i]?.Trim() : null;
                e.Merheleler.Add(new MehkemeMerhelesi { MerheleTipi = MerheleTipi.MehkemeIclasi, Tarix = t.Value, Qeyd = saat, YaradilmaTarixi = DateTime.Now });
            }
        }
        await _uow.Repository<MehkemeIsi>().YaratAsync(e);
        await _uow.YaddaSaxlaAsync();
        return e.Id;
    }
